<template>
  <div class="aipk stockpk">
    ...
    <div class="card aipk-intro">
      ...
      <p class="card-desc">
        让 7 个真实大模型基于 ALLFUND.CN 股票靠谱指数（stock_scores）真实行情数据，先选二级行业、再在该行业内选个股，
      ...
      <div class="aipk-src">数据来源：ALLFUND.CN 股票靠谱指数库（真实行情，非模拟）</div>
    ...
    <div class="aipk-models">
      <div class="aipk-model" v-for="m in orderedModels" :key="m.id" :style="{ borderTopColor: m.color }">
        ...
        <div class="aipk-model-ret" v-if="m.mode !== 'pending'" :class="retClass(modelReturns[m.id]?.return_1y)">
          近1年组合收益 {{ fmtRet(modelReturns[m.id]?.return_1y) }}
        </div>
    ...
    <div class="card aipk-pk">
      ...
      <div class="aipk-rank-note" v-if="ranking.length < orderedModels.length">
        注：{{ orderedModels.length - ranking.length }} 个模型因成分股票该周期暂无行情数据，该周期暂无数据
      </div>
      ...
      <div class="aipk-tl-model" v-for="m in orderedModels" :key="m.id">
        ...
        <template v-else>
          <div class="aipk-tl-layer">
            <span class="aipk-tl-tag">第一层 · 基于 stock_scores 选二级行业（真实收益/回撤/夏普统计）</span>
            <p class="aipk-tl-text">{{ m.category_logic || '—' }}</p>
          </div>
          <div class="aipk-tl-layer">
            <span class="aipk-tl-tag">第二层 · 个股逻辑（多维度分析）</span>
        ...
        <div class="aipk-manage-panel">
          ...
          <div class="aipk-manage-body">
            ...
            <form class="aipk-manage-form" @submit.prevent="saveModel">
              ...
              <label class="aipk-field">
                <span class="aipk-field-label">选股策略提示词 / Prompt</span>
                <textarea
                  class="aipk-input aipk-textarea" v-model="form.system_prompt" rows="5"
                  placeholder="系统提示词，用于指导模型如何选股票"
                ></textarea>
                <span class="aipk-field-hint">为不同模型设置不同的选股策略，例如风险偏好、行业侧重等。</span>
              </label>
...
const DEFAULT_SYSTEM_PROMPT =
  '你是一个专业的股票分析师。请根据以下指标（收益、最大回撤、夏普比率、行业地位、市值等）综合评估股票，' +
  '挑选 5 只股票并给出每只的选股理由，每只建议等权配置（20%）。' +
...
const stockReturns = ref({})    // { code: { ...returns } }
...
const RETURN_COLS = [
  ...
  { key: 'return_3y', label: '近3年' },
]
const STRICT_COLS = { return_3y: true }
...
const chartPeriod = ref('return_1y')
...
const modelReturns = computed(() => {
  ...
  for (const m of models.value) {
    ...
    for (const col of RETURN_COLS) {
      const vals = picks
        .map(p => ({ w: p.weight || 0, v: stockReturns.value[p.code]?.[col.key] }))
      // 严格列：任一成分缺失 → 整列 --
...
// 取代旧的固定「人设」文案：现在每个模型都按自身推理逻辑自主选股
...
async function loadAll() {
  loading.value = true
  try {
    const { data: m } = await supabase.from('stock_pk_models').select('*').eq('enabled', true)
    models.value = (m || []).map(x => ({ ...x, is_custom: false }))
    // 合并用户自建模型（出现在「模型阵容」中）
    await loadUserModels()
    const { data: p } = await supabase.from('stock_pk_picks').select('*').order('period_month', { ascending: false })
    const byModel = {}
    for (const row of (p || [])) {
    ...
    if (codes.size) {
      const { data: fr } = await supabase.from('stock_scores')
        .select('code,return_1m,return_3m,return_6m,return_1y,return_3y,daily_change')
        .in('code', [...codes])
      const map = {}
      ;(fr || []).forEach(f => { map[f.code] = f })
      stockReturns.value = map
    }
  } catch (e) {
    console.error('[StockPkPanel]', e)
  } finally {
    loading.value = false
    await nextTick()
    renderChart()
...
function renderChart() {
  ...
  const periods = CHART_PERIODS.map(p => p.key)   // [return_1m, return_3m, return_6m, return_1y, return_3y]
...
function drawLineupPoster(ctx, W, pad, headerH) {
  ...
  for (const m of orderedModels.value) {
    ...
    if (!picks.length) {
      ...
      ctx.fillText('暂无选股数据', pad + 14, y + 4); y += 30
    } else {
      picks.forEach((f, i) => {
        ctx.fillStyle = '#f6f8fb'
...
function drawTimelinePoster(ctx, W, pad, headerH) {
  ...
  for (const m of orderedModels.value) {
    ...
    ctx.fillText('第一层 · 二级行业选择逻辑', pad + 10, y)
    y += 24
...
async function generateShareImage(section) {
  ...
  try {
    ...
    ctx.fillText('识别二维码，查看靠谱指数与股票组合 PK', W / 2, qrY + qrSize + 44)
    ...
    console.error('[StockPkPanel] generateShareImage error', e)
  } finally {
...
async function loadUserModels() {
  ...
  try {
    ...
    console.error('[StockPkPanel] loadUserModels', e)
  }
}
...
async function saveModel() {
  ...
  try {
    ...
    console.error('[StockPkPanel] saveModel', e)
...
async function deleteModel(m) {
  ...
  try {
    ...
    console.error('[StockPkPanel] deleteModel', e)
...
async function toggleModelActive(m) {
  ...
  try {
    ...
    console.error('[StockPkPanel] toggleModelActive', e)
  }
}
...
.aipk-tl-fund-ind { font-weight: 400; font-size: 12px; color: var(--text-secondary); margin-left: 6px; }
